...
app.get('/productos',(req,res)=>{
    const sql = `
        ...
        WHERE p.Indicador_Urgencia IS NULL AND p.Stock > 0
...
app.get('/ofertas',(req,res)=>{
    const sql = `
        ...
        INNER JOIN RESTAURANTE r
        ON p.ID_Restaurante = r.ID_Restaurante
        WHERE p.Indicador_Urgencia IS NOT NULL AND p.Stock > 0
        ORDER BY p.ID_Producto
    `;
        ...
        (err,result)=>{
            ...
            db.query(
                ...
                (err)=>{
                    ...
                    const promesasStock = productos.map(prod => {
                    ...
                    Promise.all(promesasStock)
                        ...
                        .catch(err => {
                            res.json({ success:false, mensaje:'Error actualizando stock' });
                        });
                }
            );
        }
...
app.get('/pedidos/:id_usuario', (req, res) => {
    ...
    const sql = `
        SELECT p.ID_Pedido, 
               p.Nombre_Cliente_Form, 
               p.Total_Pagar, 
               p.Estado, 
               GROUP_CONCAT(CONCAT(pr.Nombre_Producto, ' (x', d.Cantidad, ')') SEPARATOR ', ') as Productos,
